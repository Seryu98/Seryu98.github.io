...
[10. 알파버전 개발 요구사항](#9-알파버전-개발-요구사항)  

[11. 알파버전 개발작업 일정](#9-알파버전-개발작업-일정)  

[12. 알파버전 주차별 구현 과정](#9-알파버전-주차별-구현-과정)  
...
### 게임 화면  
1. 게임화면에서는 상단에 스테이지, HP, 코인수가 나오고 하단에는 상점 ui가 나온다.  
2. 왼쪽 상단에 일시정지 버튼이 있고 플레이어가 누르면 게임이 정지한다. 스페이스 버튼 또한 가능  
3. 터렛을 실치 할 수 있는 맵 노드가 있고 설치를 위해 마우스를 가져다대면 회색으로 강조되어 유저가 인지할 수 있게한다.  
4. 조작은 마우스를 이용해 화면 하단 상점에서 원하는 터렛을 클릭하여 설치한다.  
5. 상점에는 5종류의 터렛이 있고 각자 정해진 가격이 있다.  
6. 설치된 터렛을 클릭하면 강화ui가 나오고 공격력이나 공격속도를 코인으로 강화할 수 있다.  
7. 마우스를 상하좌우로 화면테두리로 이동하거나 wsad키를 이용해 화면 카메라를 이동할 수 있다.  
8. 길의 시작지점에서 몬스터가 스테이지에 맞게 생성되고 터렛에 죽지 않은 몬스터가 길 끝에 도달시 HP가 깎인다.  
9. 게임의 종료 조건은 HP가 0이되면 종료한다.  
10. 게임 종료 시 팝업이 뜬다. 팝업에는 잡은 몬스터 수, 플레이타임, 클리어한 최종스테이지, 메인화면 버튼이 있다.  
11. 메인화면 버튼을 누르면 게임화면에서 메인화면으로 이동한다.  

### 몬스터  
1. 스테이지가 올라갈때마다 몬스터의 수가 늘어나며 4스테이지 간격을 같은 몬스터가 나온다. 5스테이지 마다 좀더 단단하고 빠른 보스 몬스터가 나온다.  
2. 몬스터 애니메이션  

### 터렛  
1. 설치 된 터렛은 특정범위에 몬스터가 들어오면 총구를 몬스터의 위치로 회전하며 공격한다.  
2. 모든 공격은 유도된다.  
3. 재화를 사용해 강화할 수 있으며 클릭하여 제거도 가능하다.  
4. 터렛의 종류마다 효과와 공격력, 공격속도, 범위가 다르다.  
5. 버프타워는 몬스터를 느리게 만들고 주변 터렛을 강화한다.  
...
3. 터렛을 실치 할 수 있는 맵 노드가 있고 설치를 위해 마우스를 가져다대면 회색으로 강조되어 유저가 인지할 수 있게한다.  
4. 조작은 마우스를 이용해 화면 하단 상점에서 원하는 터렛을 클릭하여 설치한다.  
5. 상점에는 4종류의 터렛이 있고 각자 정해진 가격이 있다.  
6. 마우스를 상하좌우로 화면테두리로 이동하거나 wsad키를 이용해 화면 카메라를 이동할 수 있다.  
7. 길의 시작지점에서 몬스터가 스테이지에 맞게 생성되고 터렛에 죽지 않은 몬스터가 길 끝에 도달시 HP가 깎인다.  
8. 게임의 종료 조건은 HP가 0이되면 종료한다.  
9. 게임 종료 시 팝업이 뜬다. 팝업에는 잡은 몬스터 수, 플레이타임, 클리어한 최종스테이지, 메인화면 버튼이 있다.  
10. 메인화면 버튼을 누르면 게임화면에서 메인화면으로 이동한다.  
...
1. 설치 된 터렛은 특정범위에 몬스터가 들어오면 총구를 몬스터의 위치로 회전하며 공격한다.  
2. 모든 공격은 유도된다.  
3. 터렛의 종류마다 효과와 공격력, 공격속도, 범위가 다르다.
...
## 8주차(게임화면의 맵과 카메라 구현)
1. 터렛을 실치 할 수 있는 맵 노드가 있고 설치를 위해 마우스를 가져다대면 회색으로 강조되어 유저가 인지할 수 있게한다.
2. 마우스를 상하좌우로 화면테두리로 이동하거나 wsad키를 이용해 화면 카메라를 이동할 수 있다.

## 9주차(UI관련 구현)
1. 게임화면에서는 상단에 스테이지, HP, 코인수가 나오고 하단에는 상점 ui가 나온다.
2. 왼쪽 상단에 일시정지 버튼이 있고 플레이어가 누르면 게임이 정지한다. 스페이스 버튼 또한 가능.
3. 하단 상점에는 4종류의 터렛이 있고 각자 정해진 가격이 있다.
4. 조작은 마우스를 이용해 화면 하단 상점에서 원하는 터렛을 클릭하여 맵에 설치한다.
...
1. 길의 시작지점에서 몬스터가 스테이지에 맞게 생성되고 터렛에 죽지 않은 몬스터가 길 끝에 도달시 HP가 깎인다.
2. 게임의 종료 조건은 HP가 0이되면 종료한다.
3. 게임 종료 시 팝업이 뜬다. 팝업에는 잡은 몬스터 수, 플레이타임, 클리어한 최종스테이지, 메인화면 버튼이 있다.

## 13주차(메인씬 추가 및 UI상호작용)
1. 게임 종료 팝업에서 메인화면 버튼을 누르면 게임화면에서 메인화면으로 이동한다.
2. 메인화면, 게임화면, 결과화면 3가지 화면이 있다.
3. 메인화면에는 시작하기, 나가기 2가지 버튼이 있다.
4. 시작하기 버튼을 누르면 게임화면으로 이동하고 나가기 버튼을 누르면 게임이 꺼진다.
