...
1-8.  
1)마우스와 키를 이용하여 상하좌우로 화면 이동을 할 수 있게 카메라를 구현  
2)마우스 휠키를 이용해 화면을 확대하고 축소 할 수 있는 기능을 구현, 스크롤 시 높이의 최대와 최소를 정해주어 땅으로 들어가거나 한도없이 올라가지 않게 만듬  
3)esc키를 누르면 카메라 고정, 다시 클릭 시 풀림  
...
1)스테이지 수UI(몬스터가 생성되면 숫자가 올라감)  
2)체력(초기 체력은 20)  
3)코인(초기 코인은 450이고 상점에서 터렛 설치 시 지정된 금액만큼 차감)  
4)몬스터 생성 텀(게임이 시작하면 3.5초 후 첫 몬스터가 생성되고 그 후에는 5.5초마다 몬스터 스폰)  
5)상점(카메라이동을 따라가게 오버레이로 만들었고 터렛 이미지와 금액text를 배치)  
...
3-4-4. 독터렛 - 피격당한 적이 지속데미지(다시공격시 갱신)(0%)  
