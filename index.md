...
2-14. 게임 종료 시 팝업이 뜬다. 팝업에는 클리어한 최종스테이지, 메인화면 버튼이 있다.  
...
4-4-4. 독터렛 - 피격당한 적이 지속데미지(다시공격시 갱신) or 밟으면 지속적인 데미지를 주는 장판  
...
1-13. 게임 종료 시 팝업이 뜬다. 팝업에는 클리어한 최종스테이지, 메인화면 버튼이 있다.(2-14)  
...
3-4-4. 독터렛 - 피격당한 적이 지속데미지(다시공격시 갱신) or 밟으면 지속적인 데미지를 주는 장판(4-4-4)  
...
## 8주차(게임화면의 맵과 카메라 구현)  
1-3. 터렛을 설치 할 수 있는 맵 노드가 있고 설치를 위해 마우스를 가져다대면 회색으로 강조되어 유저가 인지할 수 있게한다.   
1-8. 마우스를 상하좌우로 화면테두리로 이동하거나 wsad키를 이용해 화면 카메라를 이동할 수 있다.  
...
1-5. 하단 상점에는 4종류의 터렛이 있고 각자 정해진 가격이 있다.  
...
3-4-4. 독터렛 - 피격당한 적이 지속데미지(다시공격시 갱신) or 밟으면 지속적인 데미지를 주는 장판  
3-4-6. 터렛의 종류마다 효과와 공격력, 공격속도, 범위가 다르다.  

## 12주차(게임 종료 조건과 나머지 UI)
1-12. 게임의 종료 조건은 HP가 0이되면 종료한다.  
1-13. 게임 종료 시 팝업이 뜬다. 팝업에는 클리어한 최종스테이지, 메인화면 버튼이 있다.  
...
## 8주차(게임화면의 맵과 카메라 구현)  
1-3. ~~터렛을 설치 할 수 있는 맵 노드가 있고 설치를 위해 마우스를 가져다대면 회색으로 강조되어 유저가 인지할 수 있게한다.~~(100%)  
1-8. ~~마우스를 상하좌우로 화면테두리로 이동하거나 wsad키를 이용해 화면 카메라를 이동할 수 있다.~~(100%)  

### 작업내용  
1-3. 3d 오브젝트로 배열하여 노드 생성, 각각의 노드에 마우스를 올릴 시 색이 회색으로 변하여 유저에게 강조, 나중에 상점ui에서 터렛 설치시 사용  
1-8.  
-마우스와 키를 이용하여 상하좌우로 화면 이동을 할 수 있게 카메라를 구현  
-마우스 휠키를 이용해 화면을 확대하고 축소 할 수 있는 기능을 구현, 스크롤 시 높이의 최대와 최소를 정해주어 땅으로 들어가거나 한도없이 올라가지 않게 만듬  
-esc키를 누르면 카메라 고정, 다시 클릭 시 풀림  
...
## 9주차(UI관련 구현)  
1-1. 게임화면에서는 상단에 스테이지, HP, 코인수가 나오고 하단에는 상점 ui가 나온다.(60%)  
1-2. ~~왼쪽 상단에 일시정지 버튼이 있고 플레이어가 누르면 게임이 정지한다. 스페이스 버튼 또한 가능~~(100%)  
1-4. 조작은 마우스를 이용해 화면 하단 상점에서 원하는 터렛을 클릭하여 설치한다.(50%)  
1-5. ~~상점에는 4종류의 터렛이 있고 각자 정해진 가격이 있다.~~(100%)  
1-11. ~~총HP는 20, 스테이지는 50~~(100%)  

### 작업내용  
1-1. 상단에 스테이지,체력,코인 UI를, 하단에는 상점UI를 배치하였으며 각각에 맞는 스크립트를 넣어주었다. 상점 스크립트는 아직 부족  
1-2. 일시정지 버튼과 SPACE 키 둘다 작동하게 만들었으며 누르면 게임이 일시정지하고 그사이에 터렛을 배치할 수 있게 만들었다.  
1-4. 버튼형식으로 만들어서 클릭하여 원하는 노드에 다시 클릭하는것으로 설치되는 기능은 만들었지만 아직 터렛 에셋을 못넣었다.  
1-5. 상점UI에 4가지 각 터렛에 맞는 금액을 설정하여 스크립트와 인스펙터창에 적용해주었다.  
1-11. 각 수치에 맞게 UI에 적용하였다.  
...
## 10주차(몬스터 웨이브 시스템(스테이지))  
1-9. ~~길의 시작지점에서 몬스터가 스테이지에 맞게 생성된다.~~(100%)  
1-10. ~~터렛에 죽지 않은 몬스터가 길 끝에 도달시 HP가 깎인다.~~(100%)  
2-1. 스테이지가 올라갈때마다 몬스터의 수가 늘어나며 4스테이지 간격을 같은 몬스터가 나온다.(60%)  
2-2. 5스테이지 마다 좀더 단단하고 빠른 보스 몬스터가 나온다.(0%)  
2-3. 몬스터 애니메이션(0%)  

9주차 요구사항 부분  
1-1. ~~게임화면에서는 상단에 스테이지, HP, 코인수가 나오고 하단에는 상점 ui가 나온다.~~(100%)  
1-4. ~~조작은 마우스를 이용해 화면 하단 상점에서 원하는 터렛을 클릭하여 설치한다.~~(100%)  

### 작업내용  
9주차  
1-1. 스테이지 수, 체력, 코인, 몬스터 생성 텀, 상점 UI를 하단으로 배치하였고 각 항목에 맞는 스크립트를 넣었다.  
-스테이지 수UI(몬스터가 생성되면 숫자가 올라감)  
-체력(초기 체력은 20)  
-코인(초기 코인은 450이고 상점에서 터렛 설치 시 지정된 금액만큼 차감)  
-몬스터 생성 텀(게임이 시작하면 3.5초 후 첫 몬스터가 생성되고 그 후에는 5.5초마다 몬스터 스폰)  
-상점(카메라이동을 따라가게 오버레이로 만들었고 터렛 이미지와 금액text를 배치)  
1-4. 만들어진 상점UI에 원하는 터렛을 클릭하고 다시 노드에서 원하는 지점을 클릭하면 선택한 터렛이 설치되게 만들었다.  

10주차  
1-9.  
-시작지점과 끝지점에 오브젝트를 만들어 줌.  
-몬스터가 지나갈 길에 꺾이는 부분마다 웨이포인트를 만들어주어 시작지점에서 생성된 몬스터가 웨이포인트를 지나서 끝지점 오브젝트에 도달하게 만듬.  
-몬스터가 정해진 생성 텀 후에 다시 생성되며 스테이지UI가 올라가고 생성 텀UI는 초기화.  
1-10. 몬스터가 끝오브젝트에 도달하면 하단UI의 체력이 깎이게 만들었다.  
2-1. 몬스터가 생성되며 스테이지 수가 올라가면 다음 생성때 생성되는 몬스터의 수가 계속해서 1마리씩 늘어나게 만들었다.  
아직 4스테이지마다 다른 몬스터가 생성되게 만들지 못하였고 지금은 한종류의 몬스터만 나오게 되어있다.  
2-2. 2-1과 마찬가지로 5스테이지마다 보스몬스터가 생성되게 해야하는데 아직 만들지 못하였다.  
2-3. 애니메이션을 넣으면 몬스터스폰에 문제가 생겨서 아직 넣지 못하였다.  
...
## 11주차(터렛)  
3-1. ~~설치 된 터렛은 특정범위에 몬스터가 들어오면 총구를 몬스터의 위치로 회전하며 공격한다.~~(100%)  
3-2. ~~모든 공격은 유도된다.~~(100%)  
3-3. 터렛 종류 - 기본터렛, 화염터렛, 폭발터렛, 독터렛 4종류(30%)  
3-4. 터렛 기능 구현  
3-4-1. 기본터렛 - 일반 총알 발사(80%)  
3-4-2. 화염터렛 - 근접한 적에게 화염 발사(지속딜)(0%)  
3-4-3. 폭발터렛 - 피격당한 적 주변에게 광격 데미지(80%)  
3-4-4. 독터렛 - 피격당한 적이 지속데미지(다시공격시 갱신) or 밟으면 지속적인 데미지를 주는 장판(0%)  
3-4-6. 터렛의 종류마다 효과와 공격력, 공격속도, 범위가 다르다.(50%)  

10주차 부분  
2-3. ~~몬스터 애니메이션~~(100%)  

### 작업내용  
3-1. 터렛 오브젝트를 머리부분과 베이스 부분으로 나누고 터렛의 인식범위를 만들어 주어 범위내에 태그가 Enemy인 오브젝트가 들어오면 머리부분이 회전하며 적을 조준한다. 또한 머리부분에 있는 총구앞에서 총알이 발사되게하여 조준한 적을 공격하게 만들었다.  
3-2. 가장 가까운 적을 우선적으로 공격하고 탄환이 적에게 유도되어 타격한다.  
3-3. 일단 각각 종류에 맞는 터렛을 상점에서 구매하여 설치할 수 있게 하였지만 아직 터렛의 기능 및 효과가 완전하지 않다. 파티클 효과를 임시로 만들어서 사용중(원하는 효과에셋을 찾을때까지)  
3-4-1. 기본터렛은 단일적을 타격하게 만들었다. 공격속도가 빠름  
3-4-2. 화염터렛은 일단 불꽃파티클을 넣어놨는데 아직 지속딜을 넣는 기능을 구현하지 못하였다.  
3-4-3. 폭발터렛은 인식범위 외에도 타격 시 폭발하는 범위를 만들어서 여러 몬스터를 동시에 타격할 수 있게 만들었다. 기본터렛보닥 공격속도 느림.  
3-4-4. 독터렛은 아직 구현하지 못하였다.  
3-4-5. 기본터렛은 공격력50 공격속도70 폭발범위0 / 화염터렛은 여러범위를 지속적으로 공격하게 만들예정 / 폭발터렛은 공격력80 공격속도30 폭발범위10 / 독터렛은 단일 공격이지만 도트딜이 들어가고 다시 타격시 갱신되게 만들예정  

10주차  
2-3. 몬스터 오브젝트에 걷기 애니메이션을 넣어주고 각 꺾이는 지점에 있는 웨이포인트를 타겟으로 바라보게 만들어주었다. 몬스터 속도는 일단 10으로 지정  
...
## 12주차(게임 종료 조건과 나머지 UI)  
10주차  
2-1. ~~스테이지가 올라갈때마다 몬스터의 수가 늘어나며 4스테이지 간격을 같은 몬스터가 나온다.~~(100%)  
2-2. ~~5스테이지 마다 좀더 단단하고 빠른 보스 몬스터가 나온다.~~(100%)  
...
12주차  
1-12. ~~게임의 종료 조건은 HP가 0이되면 종료한다.~~(100%)  
1-13. ~~게임 종료 시 팝업이 뜬다. 팝업에는 잡은 몬스터 수, 플레이타임, 클리어한 최종스테이지, 메인화면 버튼이 있다.~~(100%)  

### 작업내용  
10주차  
2-1.  
-스테이지가 올라갈때 마다 몬스터의 수가 늘어나고 최대 4마리인 웨이브까지 스폰 후 다른 종류의 몬스터가 스폰되게 바뀜.  
-최대 50스테이지까지 10종류의 일반 몬스터(1~4마리), 10종류의 보스몬스터(1마리)가 스폰된다.  
-몬스터에게는 이동속도와 체력, 잡을시 나오는 코인이 각각 다르게 정해져있고 스테이지가 높을 수록 수치도 높아진다.  
2-2. 5스테이지마다 총 10종류의 보스몬스터가 스폰되며 보스몬스터는 일반몬스터보다 크기가 크며 이동속도, 체력이 높고 잡을시 나오는 코인의 수가 더 많다.  

11주차  
이번 주차에 작업하지 못하였다.  

12주차  
1-12. 게임 종료 조건을 체력이0이 될 때와 모든 몬스터를 잡고 스테이지50을 클리어했을때 2종류로 나눴고 조건을 만족하면 진행되던 게임이 정지하게 만들었다.  
1-13. 위에 조건을 만족하여 게임이 정지하면 각 조건에 맞는 ui가 팝업되고 ui에는 종료시점의 스테이지 수와 종료버튼 메인화면버튼이 있다.  
...
11주차  
3-4. 터렛 기능 구현  
3-4-2. 화염터렛 - 근접한 적에게 화염 발사(지속딜)(60%)  
3-4-4. 독터렛 - 피격당한 적이 지속데미지(다시공격시 갱신) or 밟으면 지속적인 데미지를 주는 장판(0%)  

13주차  
1-6. ~~설치된 터렛을 클릭하면 강화ui가 나온다.~~(100%)  
1-7. 강화ui에서 코인으로 강화할 수 있다.(0%)  
3-5. 강화ui에서 코인을 사용해 공격속도와 공격력을 강화할 수 있다.(0%)  
3-6. ~~클릭하여 강화ui에서 제거가 가능하다.~~(100%)  

추가작업(알파버전X)  
1-1. ~~메인화면, 게임화면, 결과화면 3가지 화면이 있다.~~(100%)  
1-2. ~~메인화면에는 시작하기, 나가기 2가지 버튼이 있다.~~(100%)  
1-3. ~~시작하기 버튼을 누르면 게임화면으로 이동하고 나가기 버튼을 누르면 게임이 꺼진다.~~(100%)  

### 작업내용  
11주차  
3-4.  
3-4-2. 화염터렛은 일단 화염방사 파티클을 부여해주고 루프기능으로 계속해서 화염이 발사되며 일정시간마다 데미지 타격이 들어가게 만들었다. 에임이 계속해서 가까운 적에게 돌아가게 코딩되어있어서 화염이 흩뿌리듯이 발사되는 문제가 있고 공격을 할때만 파티클이 실행되야하는데 루프기능에 의해 계속해서 화염이 발사되는점이 수정되어야한다.  
3-4-4. 독터렛은 발사한 공격이 장판 형식으로 바닥에 깔려서 밟으면 지속적인 데미지를 주게 하려했는데 구현하지 못하였다. 현재는 폭발터렛과 동일한 기능의 터렛이 설치된다.  

13주차  
1-6, 3-6. 설치된 터렛을 클릭하면 강화ui와 판매(제거)ui가 나오게 만들었다. 또한 다른 종류의 터렛을 상점에서 클릭하거나 설치된 터렛을 클릭하면 ui가 꺼지게 했다.  
1-7, 3-5. ui를 클릭할 수 있게 만들어는 놓았는데 코드는 일부 짜놨지만 아직 강화하는 기능은 구현하지 못하였고 판매하는 기능은 구현하였다. 판매시 구매금액의 절반을 돌려주게 만들었으며 추가로 금액이 부족하여 구매가 불가능 할 경우 설치하려고 마우스를 노드에 가져다 대면 노드가 회색에서 빨간색으로 변하여 설치할 수 없다는 것을 유저가 인식 할 수 있게 만들었다.  

추가작업  
결과화면이 따로 있지는 않고 결과팝업ui로 대체했고 메인화면만 추가로 만들었다. 메인화면에 있는 버튼에 애니메이션을 추가하였다.  
