...
1-9.  
1) 시작지점과 끝지점에 오브젝트를 만들어 줌.  
2) 몬스터가 지나갈 길에 꺾이는 부분마다 웨이포인트를 만들어주어 시작지점에서 생성된 몬스터가 웨이포인트를 지나서 끝지점 오브젝트에 도달하게 만듬.  
3) 몬스터가 정해진 생성 텀 후에 다시 생성되며 스테이지UI가 올라가고 생성 텀UI는 초기화.  
1-10. 몬스터가 끝오브젝트에 도달하면 하단UI의 체력이 깎이게 만들었다.  
2-1.  
몬스터가 생성되며 스테이지 수가 올라가면 다음 생성때 생성되는 몬스터의 수가 계속해서 1마리씩 늘어나게 만들었다.  
